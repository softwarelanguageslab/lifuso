...
public class CompleteCodeSnippetParser extends BaseParser<Object> {

    //-------------------------------------------------------------------------
    //  Input Line
    //-------------------------------------------------------------------------
    public Rule InputLine() {
        return ZeroOrMore(AllCode());
    }
    ...
    //-------------------------------------------------------------------------
    public Rule AllCode() {
        return FirstOf(JavaFragment(), Water());
    }

    //-------------------------------------------------------------------------
    //  Java Fragment
    //-------------------------------------------------------------------------
    public Rule JavaFragment() {
        return FirstOf(LocalVariableDeclarationStatement(), MethodInvocation(), MethodDeclaration());
    }
    ...
    // IMPORTANT: Noise is added when the semicolon (SEMI) is considered optional since the rule for variables is quite
    // broad without the semicolon symbol. Since the focus of the approach is on method calls, a filter to the noisy
    // variables can be applied and avoid an incorrect selection of the variable names.
    public Rule LocalVariableDeclarationStatement() {
        return Sequence(ZeroOrMore(Modifier()), ZeroOrMore(FirstOf(FINAL, Annotation())), pushInitialValueMap(),
                Type(), saveMatchToValueMap("VARIABLE_TYPE"),
                VariableDeclarators(),
                Optional(SEMI));
    }
    ...
    public Rule VariableDeclarator() {
        return Sequence(Identifier(), saveMatchToValueMap("VARIABLE_ID"),
                ZeroOrMore(Dim()),
    ...
    Rule FormalParameter() {
        return Sequence(ZeroOrMore(FirstOf(FINAL, Annotation())),
                Type(), pushInitialValueMap(), saveMatchToValueMap("VARIABLE_TYPE"),
                VariableDeclaratorId(), saveMatchToValueMap("VARIABLE_ID"));
    }

    Rule FormalParameterList() {
        return Sequence(FormalParameter(), ZeroOrMore(Sequence(COMMA, FormalParameter())));
    }
    ...
    Rule Creator() {
        return FirstOf(
                Sequence(Optional(NonWildcardTypeArguments()),
                        CreatedName(), saveMatchToValueMap("RECEIVER_NAME"),
                        ClassCreatorRest()),
                Sequence(Optional(NonWildcardTypeArguments()),
                        FirstOf(ClassType(), BasicType()), saveMatchToValueMap("RECEIVER_NAME"),
                        ArrayCreatorRest())
        );
    }
    ...
    Rule VariableInitializer() {
        return FirstOf(MethodInvocation(), ArrayInitializer(), Expression());
    }
    ...
    Rule ExpressionName() {
        return FirstOf(Identifier(), Sequence(Identifier(), ".", Identifier()));
    }
    ...
    //  Method Invocations
    //-------------------------------------------------------------------------
    public Rule MethodInvocation() {
        return FirstOf(
                Sequence(ExpressionName(), pushInitialValueMap(), saveMatchToValueMap("RECEIVER_NAME"),
                        OneOrMore(
                                DOT, saveMatchToValueMap("DOT"),
                                Identifier(), saveMatchToValueMap("METHOD_NAME"),
                                ZeroOrMore(TypeArguments()),
                                OneOrMore(Arguments())
                        ),
                        Optional(SEMI)),
                Sequence(pushInitialValueMap(), Primary(),
                        OneOrMore(
                                DOT, saveMatchToValueMap("DOT"),
                                Identifier(), saveMatchToValueMap("METHOD_NAME"),
                                ZeroOrMore(TypeArguments()),
                        ...
                        Optional(SEMI)),
                Sequence(SUPER, DOT, Identifier(), ZeroOrMore(TypeArguments()), OneOrMore(Arguments()), Optional(SEMI)),
                Sequence(Type(), pushInitialValueMap(), saveMatchToValueMap("RECEIVER_NAME"),
                        SUPER,
                        OneOrMore(
                                DOT, saveMatchToValueMap("DOT"),
                                Identifier(), saveMatchToValueMap("METHOD_NAME"),
    ...
    public Rule MethodDeclaration() {
        return Sequence(ZeroOrMore(Modifier()), MethodHeader());
    }

    Rule MethodHeader() {
        return FirstOf(
                Sequence(Result(), MethodDeclarator(), ZeroOrMore(THROWS)),
                Sequence(TypeArguments(), ZeroOrMore(Annotation()), Result(), MethodDeclarator(), ZeroOrMore(THROWS))
        );
    }
    ...
    Rule MethodDeclarator() {
        return Sequence(Identifier(),
                LPAR,
                ZeroOrMore(Sequence(ReceiverParameter(), COMMA)),
                ZeroOrMore(FormalParameterList()),
                RPAR,
                ZeroOrMore(Dim()));
    }

    Rule ReceiverParameter() {
        return Sequence(ZeroOrMore(Annotation()), Type(), ZeroOrMore(Sequence(Identifier(), DOT)), THIS);
    }

    Rule Result() {
        return FirstOf(Type(), VOID);
    }
    ...
    boolean saveAttrToValueMap(String key, Object value) {
        ArrayList<Object[]> values = (ArrayList<Object[]>) peek();
